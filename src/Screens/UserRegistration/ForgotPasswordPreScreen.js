...
const ForgotPasswordPreScreen = () => {
      ...
      .then((result) => {
        if (result.status === "SUCCESS" && result.status_code === 200) {
          navigate("/forgotpassword", {
            state: { Mobile: mobile, Email: email, DialCode: "+91" },
          });
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{`Reset Password | Rozgaar India`}</title>
        <meta
          name="description"
